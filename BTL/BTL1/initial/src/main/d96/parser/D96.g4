...
// Class Body
class_body: LP class_stmt_list? RP;
...
arraylist: array COMMA arraylist | array;
...
bvardecl: VAL bidlist COLON dttyp | VAL bid_vas;
bvaldecl: VAR bidlist COLON dttyp | VAR bid_vas;

// LHS + Variable assignment
scalar: scalar DOT ID | ID DCOLON DOLLARID | sd? ID;
index_scalar: index_scalar LC expr RC | scalar;
...
// Foreach statement
forstmt: FOREACH LB scalar IN expr DDOT expr (BY expr)? RB blockstmt;

// If Elseif Else statement
ifstmt: ifpart elseif_part? else_part?;
ifpart: IF LB expr RB blockstmt;
elseif_part: ELSEIF LB expr RB blockstmt elseif_part | ELSEIF LB expr RB blockstmt;
...
term7: term7 LC expr RC | term8;
term8: term8 DOT (ID | func) | sd (ID | func) | term9 ;
term9: ID DCOLON (DOLLARID | dollarfunc) | term10;
term10: (NEW ID LB valuelist? RB) | array
        | ID | (LB expr RB) | FALSE
        | TRUE | FLOATLIT | STRINGLIT 
        | INTLIT | ZERO | NULL;
sd: SELF DOT;
...
// ARRAY
array: (ARRAY LB (valuelist | arraylist) RB ) | (ARRAY LB RB);
