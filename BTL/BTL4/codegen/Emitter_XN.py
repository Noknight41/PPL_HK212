...
class Emitter():
    ...
    *   @param name the symbol entry of the variable.
    ...
    def emitWRITEVAR(self, name, inType, index, frame):
        ...
        if type(inType) is IntType or type(inType) is BoolType: #Edit BoolType
            return self.jvm.emitISTORE(index)
        elif type(inType) is FloatType:
            return self.jvm.emitFSTORE(index) #Edit FloatType
        elif type(inType) is ArrayPointerType or type(inType) is cgen.ClassType or type(inType) is StringType:
    ...
    *   @param isFinal true in case of constant; false otherwise
    ...
    def emitATTRIBUTE(self, lexeme, in_, isFinal, value):
        ...
        return self.jvm.emitSTATICFIELD(lexeme, self.getJVMType(in_), False) #Edit False
    ...
    *   @param in the type of the operands.
    ...
    def emitNOT(self, in_, frame):
        ...
        result = list()
        result.append(self.emitIFTRUE(label1, frame)) #Edit self
        result.append(self.emitPUSHCONST("true", in_, frame)) #Edit self
        result.append(self.emitGOTO(label2, frame)) #Edit self
        result.append(self.emitLABEL(label1, frame)) #Edit self
        result.append(self.emitPUSHCONST("false", in_, frame)) #Edit self
        result.append(self.emitLABEL(label2, frame)) #Edit self
        return ''.join(result)
    ...
    *   generate ior
    ...
    def emitRELOP(self, op, in_, trueLabel, falseLabel, frame):
        ...
        if op == ">":
            ...
            #result.append(self.emitGOTO(trueLabel)) #Edit 
        elif op == ">=":
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        elif type(in_) is FloatType: #edit FloatType
            frame.pop()
            return self.jvm.emitFRETURN()
        elif type(in_) is VoidType:
            return self.jvm.emitRETURN()
        elif type(in_) is StringType: #edit StringType
            frame.pop()
            return self.jvm.emitARETURN()
    
    def emitNEWARRAY(self, in_):
        #in_: Type
        ...
        if type(in_) is IntType:
            ...
            return self.jvm.emitANEWARRAY("string")
