...
from CodeGenError import *        ### Import Error
...
class Emitter():
    ...
    def emitPUSHICONST(self, in_, frame):
        ...
        if type(in_) is int:
            ...
            if i >= -1 and i <=5:
                ...
                return self.jvm.emitLDC(""+str(i))      ### add emitLDC()
    ...
    *    @param typ the type of the constant
    ...
    def emitPUSHCONST(self, in_, typ, frame):
        ...
        elif type(typ) is FloatType:                ### add FloatType
            return self.emitPUSHFCONST(in_, frame)
        elif type(typ) is BoolType:                 ### add FloatType
    ...
    def emitALOAD(self, in_, frame):
        ...
        if type(in_) is BoolType:
            return self.jvm.emitBALOAD()       ### add BoolType
        if type(in_) is FloatType:
            return self.jvm.emitFALOAD()       ### add FloatType
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        elif type(inType) is cgen.ArrayPointerType or type(inType) is cgen.ClassType or type(inType) is cgen.StringType or type(inType) is ArrayType or type(inType) is StringType:   ### add cgen.StringType
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):           ### add Float & Bool
        #in_: Type
        #frame: Frame
...
####### add ###
    def emitNEWARRAY(self,in_,frame):
        buffer = []
        buffer.append(self.emitPUSHICONST(in_.upper - in_.lower + 1, frame))
